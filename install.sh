...
check_docker() {
  ...
  # 验证 docker daemon 是否正常运行
  if ! docker info &> /dev/null; then
    echo "错误：Docker 守护进程未运行或当前用户没有权限。请启动 Docker 或使用 sudo 运行本脚本。"
    exit 1
  fi
}
...
cleanup_local_legacy() {
  # 必须使用 --no-pager 否则 systemctl 在部分系统中会导致终端挂起等待按 q 退出
  if systemctl --no-pager list-units --full -all | grep -Fq "flux_agent.service" 2>/dev/null; then
