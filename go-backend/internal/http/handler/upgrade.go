...
const (
	githubRepo     = "Xeloan/flvx-tcppreconn"
...
// buildRawMainURL builds a URL to download a file from the main branch
// of the repository, applying the GitHub proxy if configured.
func (h *Handler) buildRawMainURL(path string) string {
	enabled, proxyURL := h.getGithubProxyConfig()
	base := fmt.Sprintf("%s/%s/%s/%s", githubRawBase, githubRepo, githubBranch, path)
...
// buildRepoCloneURL returns the git clone URL for the repository,
// applying the GitHub proxy if configured.
func (h *Handler) buildRepoCloneURL() string {
	enabled, proxyURL := h.getGithubProxyConfig()
	base := fmt.Sprintf("%s/%s.git", githubHTMLBase, githubRepo)
...
func (h *Handler) nodeUpgrade(w http.ResponseWriter, r *http.Request) {
	...
	result, err := h.wsServer.SendCommand(req.ID, "UpgradeAgent", map[string]interface{}{
		"engine":        req.Engine,
		"sourceUpgrade": true,
		"repoCloneUrl":  h.buildRepoCloneURL(),
		"branch":        githubBranch,
