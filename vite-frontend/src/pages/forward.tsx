...
export default function ForwardPage() {
  ...
  const handleEdit = (forward: Forward) => {
    ...
    setForm({
      ...
      tcpPreconn: forward.tcpPreconn === 1,
    });
    setErrors({});
    setModalOpen(true);
  };
    ...
    try {
      ...
      if (isEdit) {
        const updateData = {
          ...
          tcpPreconn: form.tcpPreconn ? 1 : 0,
        };
                  ...
                  {isAdmin && (
                    <div className="flex items-center justify-between rounded-lg border border-default-200 px-4 py-3">
                      <div className="flex flex-col gap-0.5">
                        <span className="text-sm font-medium">TCP 预连接</span>
                        <span className="text-xs text-default-400">
                          启用后使用零拷贝预连接池转发，降低首包延迟（仅限 Linux
                          节点）
                        </span>
                      </div>
                      <Switch
                        isSelected={form.tcpPreconn}
                        onValueChange={(val) =>
                          setForm((prev) => ({ ...prev, tcpPreconn: val }))
                        }
                      />
                    </div>
                  )}
