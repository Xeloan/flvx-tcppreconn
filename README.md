...
本项目不区分版本，始终从 `main` 分支拉取最新代码，本地编译运行。
...
脚本会自动克隆仓库到 `/opt/flvx-panel`，通过 Docker Compose 从源码构建镜像并启动服务。更新时再次运行脚本选择"更新"即可（自动 `git pull` + 重新构建）。
...
脚本会自动克隆仓库到 `/opt/flvx-agent-src`，安装 Go 编译器（如未安装），从源码编译 `flux_agent` 和 `tcp_pool` 并部署为 systemd 服务。更新时再次运行脚本选择"更新"即可（自动拉取最新代码 + 重新编译）。

#### PostgreSQL 部署（Docker Compose）

安装脚本会克隆仓库并根据环境自动选择对应的 Compose 配置。默认使用 SQLite，切换到 PostgreSQL 只需要配置环境变量。
...
- **Updated**: Installation scripts (`install.sh`, `panel_install.sh`) now clone from `main` and build from source.
